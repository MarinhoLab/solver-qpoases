...
Options Solver::_to_qpoases_options() const
{
    ...
    options.enableRamping = configuration_.enableRamping;
    options.enableFarBounds = configuration_.enableFarBounds;
    ...
    options.enableFullLITests = configuration_.enableFullLITests;
    options.enableNZCTests = configuration_.enableNZCTests;
    options.enableDriftCorrection = configuration_.enableDriftCorrection;
    options.enableCholeskyRefactorisation = configuration_.enableCholeskyRefactorisation;
    options.enableEqualities = configuration_.enableEqualities;
    options.terminationTolerance = configuration_.terminationTolerance;
    options.boundTolerance = configuration_.boundTolerance;
    options.boundRelaxation = configuration_.boundRelaxation;
    options.epsNum = configuration_.epsNum;
    options.epsDen = configuration_.epsDen;
    options.maxPrimalJump = configuration_.maxPrimalJump;
    options.maxDualJump = configuration_.maxDualJump;
    options.initialRamping = configuration_.initialRamping;
    options.finalRamping = configuration_.finalRamping;
    options.initialFarBounds = configuration_.initialFarBounds;
    options.growFarBounds = configuration_.growFarBounds;
    options.initialStatusBounds = configuration_.initialStatusBounds;
    options.epsFlipping = configuration_.epsFlipping;
    options.numRegularisationSteps = configuration_.numRegularisationSteps;
    options.epsRegularisation = configuration_.epsRegularisation;
    options.numRefinementSteps = configuration_.numRefinementSteps;
    options.epsIterRef = configuration_.epsIterRef;
    options.epsLITests = configuration_.epsLITests;
    options.epsNZCTests = configuration_.epsNZCTests;
    options.rcondSMin = configuration_.rcondSMin;
    options.enableInertiaCorrection = configuration_.enableInertiaCorrection;
    options.enableDropInfeasibles = configuration_.enableDropInfeasibles;
    options.dropBoundPriority = configuration_.dropBoundPriority;
    options.dropEqConPriority = configuration_.dropEqConPriority;
    options.dropIneqConPriority = configuration_.dropIneqConPriority;
    return options;
}
